...
@section('title', trans('messages.groups.routines.overview'))
...
@section('content')
    <div uk-grid>
        <div class="uk-width-1-1">
            <h1>{{ trans('messages.groups.routines.overview') }}</h1>
        </div>
        <div class="uk-width-1-1">
            <div uk-grid>
                        ...
                        @foreach($assignedRoutines as $assignedRoutine)
                            <tr>
                                <td class="ag-line-height-1 uk-table-shrink">
                                    <form class="uk-form-stacked" method="POST" action="{{ action('App\Http\Controllers\AthleteGroupsController@unassign_routine', [ $group->id ]) }}">
                                        {!! csrf_field() !!}
                                ...
                                <td>
                                    <form class="uk-form-stacked" method="POST" action="{{ action('App\Http\Controllers\AthleteGroupsController@update_assigned_routine', [ $group->id ]) }}">
                                        {!! csrf_field() !!}
                                        <input type="hidden" name="_method" value="PATCH" />
                                        <input type="hidden" name="assignedRoutine" value="{{ $assignedRoutine->id }}" />
                ...
                <table class="uk-table uk-table-divider uk-table-middle">
                    ...
                    @foreach($availableRoutines as $routine)
                        ...
                        data-ag-age="{{ $routine->ageIdAsJson() }}">
                            <td class="ag-line-height-1 uk-table-shrink">
                                <form class="uk-form-stacked" method="POST" action="{{ action('App\Http\Controllers\AthleteGroupsController@assign_routine', [ $group->id ]) }}">
                                    {!! csrf_field() !!}
                                    <input type="hidden" name="_method" value="PATCH" />
