...
@section('title', trans('messages.office.edit'))

@section('breadcrumb-link', action('App\Http\Controllers\UserOfficeController@show'))
@section('breadcrumb-title', 'Mandant')

@section('content')
    <div uk-grid>
        <div class="uk-width-1-2">
            <h1>{{ trans('messages.office.edit') }}</h1>

            <div class="uk-card uk-card-small uk-card-default uk-card-body">

                <form class="uk-form-stacked" method="POST" action="{{ action('App\Http\Controllers\UserOfficeController@update') }}" enctype="multipart/form-data">
                    {!! csrf_field() !!}
                    <input type="hidden" name="_method" value="PATCH" />

                    <div class="ag-card-nav uk-margin-bottom">
                        <a href="{{ action('App\Http\Controllers\UserOfficeController@show') }}" class="ag-icon-fab" title="Zurück">
