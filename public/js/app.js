...
$(function () {
    ...
    var viewportExpand = UIkit.heightViewport('.ag-js-viewport-expand', {
        expand:true
    });

	var search = $('.ag-js-filter-table'),
        activeLabels = [];

	var updateActiveLabels = function () {
        activeLabels = [];

        $('.ag-js-filter-label.is-active').each(function () {
            var id = $(this).data('ag-filter-id');
            var labelFor = $(this).parents('.ag-js-filter-label-wrapper').data('ag-filter-for');

            if (activeLabels[labelFor] !== undefined) {
                activeLabels[labelFor].push(id);
            } else {
                activeLabels[labelFor] = [ id ];
            }
        });
    };

    var termInTr = function (tr, searchTerm) {
        var tdElements = $(tr).children('td');
        ...
        tdElements.each(function (index, td) {
            if (filter) {
                return;
            }

            filter = td.innerHTML.toLowerCase().indexOf(searchTerm) > -1;
        });

        return filter;
    };

    var labelActive = function (tr) {
        if (!$(tr).has('td')) {
            return true;
        }

        return Object.keys(activeLabels).every(function (key) {
            var idArray = $(tr).data(key);
            if (idArray !== undefined && idArray.length > 0) {
                return idArray.some(function (id) {
                    return activeLabels[key].includes(id);
                });
            }
    ...
    var filterTable = function (trElements, searchTerm) {
        trElements.each(function (index, tr) {
            console.log(labelActive(tr))
            if (termInTr(tr, searchTerm) && labelActive(tr)) {
                $(tr).show();
            } else {
                $(tr).hide();
            }
	...
	if (search.length > 0) {
        ...
        search.on('keyup', function (e) {
            ...
            filterTable(trElements.not('thead tr'), searchTerm);
            UIkit.update(viewportExpand);
        });

        search.siblings('.uk-search-icon').on('click', function (e) {
            e.preventDefault();
        ...
        $('.ag-js-filter-label').on('click', function () {
    ...
    // show file name of selected file
    $('[type=file]').on('change', function () {
        var fileName = this.files[0].name;
        var fileNameHtml = '<div>' + fileName + '</div>';

        var $fileInputWrapper = $(this).parents('.ag-js-file-input');

        if ($fileInputWrapper.length <= 0) {
            return;
        }

        var $filePreviewWrapper = $fileInputWrapper.siblings('.ag-js-file-preview');

        if ($filePreviewWrapper.length <= 0) {
            return;
        }

        $filePreviewWrapper.html(fileNameHtml);
    });

	// add flatpickr to date fields
    $("[type=date]").each(function () {
        var $this = $(this),
            maxDate = $this.data("ag-max-date"),
            minDate = $this.data("ag-min-date");
